# let's login as usual

# make a new directory for today's lesson

mkdir extra_unix_fun
cd extra_unix_fun

# **** Fetching files with wget ****

# now we'll learn how to fetch files remotely from a web repository.
# we'll be looking at files from this mock archive
# https://tacoma.med.unc.edu/tristan/data/

# you can use the copy/paste buffer of your computer to make this easier

wget https://tacoma.med.unc.edu/tristan/data/Control_A.fastq

ls

# we now have the Control_A.fastq file in our directory

# we could fetch each file individually, maybe wildcards work?

wget https://tacoma.med.unc.edu/tristan/data/*

# to clean up:  rm -r tacoma.med.unc.edu

# Or perhaps there is an option that can help us?

wget -r https://tacoma.med.unc.edu/tristan/data/

# what does this get us?  The -r option on wget is particularly perilous

# we need a host of options to just get the directory we want
# if we just want a single directory of files, and there are no sub directories, try this

wget -r -np -nH -nd --reject="index.html*" https://tacoma.med.unc.edu/tristan/data/

# there are many ways to use wget, try looking up the options we used

# let's look at how Unix organizes these files

ls -1

# Note the order of the files, what do you notice about how the numeric values are ordered?
# This is useful to know when submitting sample names
...
# Eg, I would type:  squeue -u tristand
squeue -u <your_onyen>
